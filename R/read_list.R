...
nc_read_list <- function(ff_list, vars = NULL, cdo_output = FALSE, dim_check = 15e7) {
  ...
  # set grid_checked to false. Once we have found the grid and stored it we don't need to calculate it again. It is then reset to TRUE
  ...
  for (ff_file in ff_list) {
    ...
    if (grid_checked == FALSE) {
      ...
      # Note: in read_list this only needs to happen once
      delete_copy <- FALSE
    ...
    nc_grid <- nc_grid %>%
      dplyr::select(names(.)[names(.) %in% c("Longitude", "Latitude", "Depth")])
