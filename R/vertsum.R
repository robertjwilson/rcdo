...
#' @param expr This is a cdo expression to apply to the data before a vertical sum is calculated.
...
nc_vertsum <- function(ff, vars = NULL, vert_scale = NULL, coords = NULL, expr = NULL, na_value = NULL, out_file = NULL, zip_file = FALSE, cdo_output = FALSE, overwrite = FALSE, ...) {
  nc_vertstat(metric = "sum", ff = ff, vars = vars, vert_scale = vert_scale, coords = coords, expr = expr, na_value = na_value, out_file = out_file, zip_file = zip_file, cdo_output = cdo_output, overwrite = overwrite, ...)
}
